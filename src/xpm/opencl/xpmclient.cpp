 ...
 */
 ...
 static GetBlockTemplateContext* gGetBlockCtx = nullptr;
 static SubmitContext* gSubmitCtx = nullptr;
 static const char *gUrl = "127.0.0.1:9912";
 static const char *gUserName = nullptr;
 static const char *gPassword = nullptr;
 static const char *gWallet = nullptr;
 static int extraNonce = 0;
 static bool isBenchmark = false;
 ...
 void printHelpMessage() {
	printf("Usage: xpmclient [options]\n\n");
	printf("  -b, --benchmark        Run benchmark only and exit\n");
	printf("  -o, --url <host:port>  Primecoin RPC node (default: 127.0.0.1:9912)\n");
	printf("  -u, --user <name>      RPC username\n");
	printf("  -p, --pass <pass>      RPC password\n");
	printf("  -w, --wallet <addr>    Wallet address to receive coins\n");
	printf("      --worker-id <id>   Unique worker ID\n");
 ...
 int main(int argc, char **argv) {
	...
	if (!gUserName || !gPassword) {
	  fprintf(stderr, "Error: must specify --user and --pass\n");
	  return 1;
	}
  
	if (!gWallet && !isBenchmark) {
	  fprintf(stderr, "Error: must specify --wallet\n");
	  return 1;
	}
	...
	gGetBlockCtx = new GetBlockTemplateContext(nullptr, gUrl, gUserName, gPassword, gWallet, 4, 1, extraNonce);
	gGetBlockCtx->run();
	gSubmitCtx = new SubmitContext(nullptr, gUrl, gUserName, gPassword);
  
	Configuration cfg;
	cfg.load("config.txt");

	// Start XPMClient
	XPMClient* client = new XPMClient(nullptr);
	if (!client->Initialize(&cfg, isBenchmark, 0)) {
 ...
 void PrimeMiner::Mining(void *ctx, void *pipe) {
	 ...
	 while (!(workTemplate = gGetBlockCtx->get(0, workTemplate, &dataId, &hasChanged))) {
		 usleep(1000);
	 }
   ...
   OCL(clSetKernelArg(mSieveSearch, 7, sizeof(cl_uint), &mDepth));  
	 ...
	 while(run){
		...
		// get work
		bool reset = false;
		blktemplate_t* newWork = nullptr;
		unsigned newDataId = 0;
		bool hasChanged = false;

		while (!(newWork = gGetBlockCtx->get(mID, workTemplate, &newDataId, &hasChanged))) {
			usleep(1000);
		}

		// If the work is updated, it is marked as reset
		if (hasChanged) {
			workTemplate = newWork;
			dataId = newDataId;
			reset = true;
		}
	   ...
	   blockheader.time = workTemplate->curtime + mID;
	   ...
	   LOG_F(WARNING, "increase sieves per round to %u", mSievePerRound);
		 ...
		 bool isblock = ProbablePrimeChainTestFast(chainorg, testParams, mDepth);
		   ...
		   LOG_F(1, "GPU %d found share: %d-ch type %d", mID, chainlength, candi.type+1);
			 ...
			 gSubmitCtx->submitBlock(workTemplate, blockheader, dataId);
